      ...
      <?php
        ...
        $result = mysqli_query($db,$query);
       	while($row = mysqli_fetch_array($result)) {
       		...
       		echo("<tr><td><a href=\"profile.php?uname=".$row['user_id']."\">".$row['user_id']);
       		if($row['status'] == 0)
       		...
       		if($row['status'] == 1)
       			echo("<a href=\"update.php?action=ban&userid=".$row['user_id']."\" class=\"btn btn-mini btn-danger\">Ban</a></td></tr>\n");
       		else if($row['status'] == 0)
       			echo("<a href=\"update.php?action=unban&userid=".$row['user_id']."\" class=\"btn btn-mini btn-danger\">Unban</a></td></tr>\n");
       	}
      ?>
      </tbody>
      </table>
